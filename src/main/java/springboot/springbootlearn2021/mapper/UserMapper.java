...
public interface UserMapper {
    @Select("select username, password, authority from m_user where username = #{username}")
    User getUserByName(String username);

    @Select("select * from m_user where username = #{username} and password = #{password}")
    User login(String username, String password);

    @Insert("insert into m_user (username, password, authority) values (#{username},#{password},#{authority})")
